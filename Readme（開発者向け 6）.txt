...
＞ところでなんでこれまで対局結果による評価関数の補正って誰もやっていなかった or 誰かやっていたのかもしれないけど上手く行かなかっただったん？ 

Selene（セレネ）開発者のブログがイメージを掴みやすい？
http://msaikaishi.hatenablog.com/entry/2016/10/11/194401

2016年電王戦でもSelene（セレネ）開発者が「上手くいくからやってみたら」って、やねうら王開発者に推奨してたと思うけど、
やねうら王開発者は「与える報酬が遠すぎないか？」と懐疑的で従来の雑巾絞りのほうが良いのでは？っていう感じだったと思う。

それを上手くtanuki-エントロピー等とミックスさせたのが「elmo絞り」だと思う。
（本家のtanuki-さんもwcsc27でelmo絞りを１億局面程度やってＲ３０程度の向上が見られた様なので、勝敗の要素が何らかの効果があることは実証済み）

さらにTwitterにて
＞入玉しても引き分け有るよって教えたら弱くなったから、elmoには「入玉は正義」って教えてる。

とあったので、重要なのは入玉が強い弱いの問題ではなくて、狙って評価関数を作っている所も違う。
いままで入玉に関する雑巾絞りも、ただ入玉の棋譜集めて突っ込んどきゃいいってやり方から一線を画している。

elmo開発者のすごい所は、こういった目の付け所が違う点だと思います。


＞やねうら王のブレイクスルーって何なの？ 

このReadme（開発者向け 6）.txtの１６１行目から書いている、やねうら王2017-Early V4.41＋elmoがPonanzaに勝てた理由（個人的見解）です。

話はそれますが、開発者界隈で下ネタ合戦を繰り広げられた「ＳＭエロ河童」について、
wcsc27以降に評価関数やら探索部やらいっぱいありすぎて嬉しい悲鳴なのですが、そのベストマッチが困難を極めています。
そんな中、卑猥な名前でプッシュされた「ＳＭエロ河童」ですが、意外とベストマッチの１つじゃないかと思っています。
エロ河童評価関数はelmo評価関数より学習のデコボコが多少出来てしまいましたが、SILENT_MAJORITYは最善手を投機的に深く読む機能が付いていたはずで、
これと組み合わせて評価関数の調度良い場所に当たりまくると、瞬間的にすごい棋力を発揮する可能性があるかも。



■Ponanzaの書き込みについては、いろいろ物議をかもしているので出来るだけ補足したいと思います

①何故私がPonanzaディープラーニングが成功したと思ったのかについては、このReadme（開発者向け 6）.txtの２９６行目から書いている感じの内容です。
さらに定跡を作成している目線から言えば、単純に初手の勝率が違うのでPonanzaディープラーニングが成功した場合は普通の定跡進行のほうが、
勝率４％～６％程度向上する見込みであり、今大会出場前のPonanzaの最大のライバルは自分自身の旧Ponanzaしかいないわけで、
旧Ponanzaより勝率４％～６％程度向上すればPonanza開発者は「下手したら今後数年の未来までも含めて史上最強の将棋プログラムになるかもしれない」と
錯覚するのも当然ありうるかなと。
ただ、その選んだ道が数ヶ月オープンソース勢が研鑽してきた舞台だったということです。（敗因のひとつ）

②総合的な敗因を定跡を作成している立場で説明すると
...
この図は二次予選 6回戦 Ponanza Chainer - elmo（初対局）の３２手目ですが、この後３３手目にPonanzaが▲２八飛と引きました。
これを私は悪手と断定しています。（類似する他局面もこれを見つけ次第悪手マーカーを付けて、floodgateでもかなりの勝ち星をあげています）

３３手目が▲７七銀でPonanzaの勝ちです。２６手目△８八角成が疑問手で理由は簡単、elmoの後手番唯一の主張であった手得を放棄したからです。

これが人間の対局だと、△８八角成の指し手は悪いんじゃないんですか？と強気に▲７七銀と出るのが当たり前で、間違っても
端歩突き越し（９五歩）から何かされるんじゃないか？角が持ち駒だけど△４四角打から何かされるんじゃないか？とビビッて逃げる（▲２八飛）局面ではありません。
そんなことするくらいなら最初から横歩取り戦法なんて選択するなよと言われるレベルです。

ポーカー的に言えば手得を放棄し、完全に手札はブタなのに端歩突き越しや△４四角打見せといった掛け金をどんどんレイズしたelmoに対して、
Ponanzaがフォールドしてしまった状態です。

あれだけのハードを用意してこんな読み筋しか出来ないPonanzaを見た瞬間に、私はelmoとの格付けは済んだなと思いました。



■しかし驚きました

将棋ソフト開発者やプロ棋士の先生方は立場のある人達なので、本当は言いたいこともたくさん有るけど言えないんだろうなと思い、
私は出来るだけ思ったことを書き散らすことが出来る何処にでもいる一般人だから、ストレートに思ったことを書いとこう。
ソフト開発者が何か小さなインスピレーションを得られれば良いな程度に書いてあるのに、２chであんなに物議をかもすとは。

とりあえず戦型定跡を１サイクルして配布したら開発は終わる予定です。
そしてたぶん１ヶ月も開発から離れると２度と将棋ソフトに対抗出来るレベルの定跡は作ることが出来なくなると思います。
ずっと半年もやっていたからか、最近は序盤に限っては瞬時に急所が分かるようになってるみたいですが、将棋ソフトの棋力も格段に上がっており、
私はプロ棋士でも無いし辞めたら継続もしないので、本当に終了となるかと思います。
...
開発を辞める時期に関しては、だいぶ前に雑記.txt２１３行で書いた様にそろそろだと考えていましたので、今回のやり取りは関係ありません。
むしろ議論が活性化して、いろんなネタが転がって良かったと思っています。
（現在の将棋ソフト開発者は個人開発レベルの環境下がほとんどなので、いろんな意見から取捨選択出来るネタがあるだけでも有意義だと思います）

Readme（開発者向け 5）.txt１４７行から書いてあるように、私は当時ソフト開発者が一番ネックとなっている「評価関数作成に伴う札束ビンタ」の状態を
何とか出来ないかと自分なりに考えて書いてみました。

内容を見てわかる通り、私は評価関数のデコボコは有って当たり前、出来るだけ少労力で作成し定跡等の別のアプローチで補完出来ないか？と考えていました。
しかし、wcsc27で「elmo絞り」という圧倒的少労力で強い評価関数を作成出来る救世主が現れました。

定跡部においてもReadme（開発者向け 6）.txt９６行から書いてあるように、その役割を終えつつあると実感しました。
ただ、私が作った定跡を「なぞり型スナイプ」すれば、その指し手の理屈は分からずとも機械導出上、大幅に手間が省略できることも同時に分かりました。
（評価関数で言う教師有り学習と同じ効果）

これら上記のことから、将棋ソフト開発者の（＝将棋ソフトの発展の）ために何か最後に残しておこうと考えていました。
そんなときに現れたのが「技巧２戦型別定跡」でした。

私はいままで１つの定跡で採用率に頭を悩ましていましたが、この機能手間はかかりますが一度作ればジャンケンの要素を持たせることが出来るので、
ソフト開発者はスナイパー定跡に対抗出来るようになるし、一般ユーザーの普段使いも戦型選びが出来ると目から鱗でした。
ジャンケンの要素と言っても、グーはチョキに勝つように完全に必勝になるわけでは有りませんが、すぐに切り替えれる定跡にすべてアンチ定跡を作成するのは当然困難です。
（よほど作りこんだ定跡でなければ、少し指し手を変えるだけでも定跡は切れるから）

作成しているイメージとしては、Readme 00(技巧２戦型別).txtに書いてある感じです。
（いろんな人が定跡を作って配布してくれるのが一番なのだが……）
あとはリアル都合で本当に途中で終わる可能性もあります。そして前投稿に書いたように、ちょっとでも止まるとたぶん終了です。



■私にプログラミング力あれば、どうやって定跡を作るか

実は、将棋の定跡は駒が本格的にぶつかる瞬間までは簡単に作れます。
これは将棋の初期配置が非常に優れていて、先後対称に駒を動かすと例外を除いて千日手が見つかることを知っていれば誰でも思いつくことです。
つまり駒が本格的にぶつかる瞬間までは、ほぼすべて先手後手の速度計算で導出可能ということです。

具体例としては、二次予選 6回戦 技巧 - 大合神クジラちゃん
...
この図は、２６手目でクジラちゃんが△２四飛とぶつけた局面です。
これは私が「まふ定跡」として登録していた定跡手です。

このときの反応は、
大合神クジラちゃんの何百億ノードで「＋２００」※技巧有利
公式ニコニコでは、遠山五段「これが定跡ですか」、古作さん「まあPonderがヒットしても０秒指しになりますし」
えびちゃん生放送のコメントでは、「ハズレ定跡」「ハズレ定跡」「ハズレ定跡」「これ負けた」

と、技巧相手に総ツッコミされました。
しかし私からすれば、この局面でこの手以外は存在しません。
この後、▲３三角成、△同銀（これも定跡登録）として、技巧が▲２五歩打とやっぱごめんと謝ってきました。
（時すでに遅しですが、ここで▲２四飛が悪手であると気が付くだけでも技巧の探索部が優秀である証拠です）
これより後は本譜を確認下さい。（クジラちゃん快勝です）

何故ここで▲２四飛、△同銀じゃ先手ダメなのか？
答えは簡単で、後手は浮いた△２四銀を３三に戻す１手で陣形が整います。
先手は２３手目に定跡外しを狙った▲３八銀のせいで、陣形を整えるのに５七の地点と２八の地点をケアするのに２手必要です。

つまり速度計算上、飛車＋角＋歩２と最強の持ち駒を持った状態で後手先行になるからです。
（手元ＰＣでelmoがMultiPV３の探索深さ２４程度で▲２四飛、△同銀、▲２二歩打で＋２００とか出しましたが、これはelmoが誤評価ですぐに反省すると思います）

こういった速度計算をプログラミング出来れば、評価値なんて計算（探索）しなくても例外を除いて指し手生成出来ます。
※これは将棋ソフト毎で変動するような指し手ではないので、どのソフトにも使えるベースとなります

そして、自分のソフト専用の定跡がほしいのなら、生成した指し手に新しい評価関数で評価値を付け、エンジン側にて評価値で選択させれば良いと思いませんか？
もちろんさらに精度を求めて自己対局を繰り返し、より勝率の高い手を選抜、指し手延長しても良いでしょう。

指し手生成プログラミングイメージとしては、
先手番がひたすらに互角以上で手得していくルート、
後手番がひたすらに互角以上で手得していくルート、
超急戦のように序盤から激しくぶつかるルートは無視して簡単にプログラミング出来る範囲で、最後にまふ定跡と融合すればある程度補完出来ると思います。



■WCSC27後に開発者様の善意で配布された将棋ソフト（単体）のレーティングを見る

WCSC26→WCSC27の１年間の比較（uuunuuun氏サイトの値）

tanuki-WCSC26 	3201　→　３８３９（Ｒ＋６３８）ＫＰＰＴ型
nozomi 160529 	3244　→　３８４２（Ｒ＋５９８）ＫＰＰ型
Gikou 20160606 	3524　→　３７９４（Ｒ＋２７０）２駒＋α

改めて見ると、とんでもないですね。

ここにSDT4から配布された「読み太」や、評価関数等のパーツ配布の「Qhapaq」「elmo」を加えると、

Yomita-wcsc27 	3681　※これは定跡ＯＦＦなので、定跡込みなら推定３７８０程度か。
YO4.53/Qhapaq-wcsc27　3885　※やね探索部とＳＭのレート差から、大会中は推定３７８０程度か。
YO4.53/elmo-wcsc27　3984　※大会中はやね探索部4.41相当とレート測定に定跡を使ってなかったりしているので正確なＲは分からないが、頭ひとつ抜けている。

こうやって見ると将棋ソフト単体では、Ｒ３８００程度の所に「tanuki-」「nozomi」「技巧」「読み太」「Qhapaq」がいて、
WCSC27の大会中はハードの差や、組み合わせの妙があったことが分かる。



■技巧２戦型別　03_横歩取りを作っているのだけど

先手を最善手だけで調整してみると、元々elmo定跡はまふ定跡の疑問手を的確にスナイプしていたので、elmo同士ならelmoに先手勝率７５％程度になった。
が、スナイパー定跡を作りたいわけじゃないので６０％程度に収まる範囲で選択戦法を広げたつもりが、勝率は変わらなかった。
elmo定跡が狭すぎたため違うルートでも有利になった模様。

大会も終わったことだし、一般配布してまたスナイプ合戦になったら将棋ソフト開発者に迷惑になる？
単体ソフト開発者は、ほぼ個人開発環境下で探索部＋評価関数＋定跡と忙しいはずで、ここで定跡合戦は迷惑か？

今から後手番を調整して、最終バランスを取って一応終わり。（１０日見込み）
将棋ソフト単体でオープンソース化している開発者に原本を配布してみようか。（定跡部作成に多少手間が省けるかもしれない）
